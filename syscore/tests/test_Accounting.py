...
class Test(unittest.TestCase):
    ...
    def test_get_positions_from_forecasts(self):
        ...
        price = pd.DataFrame(
            [100, 103, 105, 106, 110, 105, np.nan, 106, 120, np.nan, 142], dt_range2
        )
    ...
    def test_get_trades_from_positions(self):
        ...
        price = pd.DataFrame(
            [100, 103, np.nan, 106, 110, 105, np.nan, 106, 120, np.nan, 142], dt_range2
        )
        ...
        np.testing.assert_almost_equal(
            trades.fill_price[:-1], [106.0, 106.0, 105.0, 106.0, 120.0, 142.0, 142.0]
        )
    ...
    def test_pandl(self):
        fx = pd.DataFrame([2.0] * 10, dt_range1)
        price = pd.DataFrame(
            [100, 103, 105, 106, 110, 105, 104.5, np.nan, 120, np.nan, 142], dt_range2
        )
            ...
            [
                pd.DataFrame(
                    dict(
                        ...
                        fill_price=[102.9, 105.5, 106.5, np.nan, 106.0],
                    ),
        ...
        np.testing.assert_almost_equal(
            ans.pandl_base[1:],
            [0.0, 10.4, 6.0, 14.0, -16.0, -9.0, 15.0, 48.0, 78.0, 40.0],
        )

        ans2 = pandl(price, trades, marktomarket=False, fx=fx)

        np.testing.assert_almost_equal(
            ans2.pandl_base[1:], [10.4, 6.0, 0.0, -2.0, 6.0, 48.0, 78.0]
        )
