...
class CsvFsbInstrumentData(futuresInstrumentData):
    """
    Get data about futures spreadbet (fsb) instruments from a special configuration used for initialising the system
    ...
    def __init__(
        ...
        log=logtoscreen("csvFsbInstrumentData"),
    ):
    ...
    def _get_instrument_data_without_checking(self, instrument_code:str) -> FuturesSpreadbetWithMetaData:
    ...
    def _add_instrument_data_without_checking_for_existing_entry(
        self, instrument_object: FuturesSpreadbetWithMetaData
    ):
        raise NotImplementedError("Can't overwrite part of .csv use write_all_instrument_data_from_df")
...
def get_instrument_with_meta_data_object(all_instrument_data: pd.DataFrame,
                                         instrument_code: str) -> FuturesSpreadbetWithMetaData:
