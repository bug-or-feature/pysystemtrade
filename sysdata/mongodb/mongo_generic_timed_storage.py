...
class mongoListOfEntriesData(listOfEntriesData):
    ...
    def _write_series_dict_for_args_dict(self, args_dict, series_as_list_of_dicts, data_class):
        __, existing_data = self. _get_series_dict_with_data_class_for_args_dict(args_dict)
        if existing_data is missing_data:
            return self._add_series_dict_for_args_dict(args_dict, series_as_list_of_dicts, data_class)
        else:
            return self._update_series_dict_for_args_dict(args_dict, series_as_list_of_dicts, data_class)

    def _add_series_dict_for_args_dict(self, args_dict, series_as_list_of_dicts, data_class):
        object_to_insert = copy(args_dict)
        object_to_insert.update(dict(entry_series = series_as_list_of_dicts,
                                data_class = data_class))
        self._mongo.collection.insert_one(object_to_insert)

        return success

    def _update_series_dict_for_args_dict(self, args_dict, series_as_list_of_dicts, data_class):
