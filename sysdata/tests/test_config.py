...
class TestConfig:
    ...
    def test_default(self):
        ...
        assert config.get_element_or_missing_data("ib_port") == 4001

    def test_custom_dir(self, monkeypatch):
        envs = {
            PRIVATE_CONFIG_DIR_ENV_VAR: 'sysdata.tests.data'
        }
        monkeypatch.setattr(os, 'environ', envs)

        config = default_config()
        assert config.get_element_or_missing_data("ib_port") == 4002
