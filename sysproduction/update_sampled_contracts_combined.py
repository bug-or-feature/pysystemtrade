...
def update_sampled_contracts_combined(instrument_list=None):
    """
    *** This differs from the upstream version, as we need to maintain two sets of
    sampled contracts, futures and FSBs, which can have different expiry dates ***


    :returns: None
    """
    with dataBlob(
        log_name="Update-Sampled-Contracts-Combined",
        csv_data_paths=dict(
        ...
        if instrument_list is None:
            update_contracts_object = updateSampledContractsCombined(data)
            ...
            update_contracts_object = updateSampledContractsCombined(
                data, instrument_list
            )
            update_contracts_object.update_sampled_contracts()


class updateSampledContractsCombined(object):
...
def _get_all_currently_sampling(fsb_code):
    with dataBlob(log_name="Test-Sampled_Contracts") as data:
        ...
        fut = get_list_of_currently_sampling_contracts_in_db(
            data, remove_suffix(fsb_code, "_fsb")
        )
        for instr in fut:
...
if __name__ == "__main__":
    ...
    update_sampled_contracts_combined(["CADJPY_fsb", "EU-BANKS_fsb", "EURO600_fsb"])
