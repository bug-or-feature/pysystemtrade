...
rsync -av $SOURCE/adjusted_prices $DEST/adjusted_prices_csv

echo "`date "+%Y-%m-%d %H:%M:%S"` Backing up multiple prices"
rsync -av $SOURCE/multiple_prices $DEST/multiple_prices_csv

echo "`date "+%Y-%m-%d %H:%M:%S"` Backing up fx prices"
rsync -av $SOURCE/fx_prices $DEST/fx_prices_csv
