...
def correlation_data(
    ...
    sliced_prices = prices[max(db_series.index[0], csv_series.index[0]):min(db_series.index[-1], csv_series.index[-1])]

    returns = pd.concat({'DB': db.diff(), 'CSV': csv.diff()}, axis=1)
    sliced_returns = returns[max(db_series.index[0], csv_series.index[0]):min(db_series.index[-1], csv_series.index[-1])]
...
def all_report():
    ...
    results = results.sort_values(by="ReturnsCorr") # PriceCorr, ReturnsCorr, StartDB, Diff
