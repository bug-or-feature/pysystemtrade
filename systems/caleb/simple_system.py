...
def pandl_for_instrument_rules():
    ...
    print(
        f"Get a list of methods. equivalent to acc_curve.gross.stats(): {acc_curve_group.gross.daily.stats()}"
    )
    ...
    print(
        f"annualised std. deviation of daily (net) returns: {acc_curve_group.daily.ann_std()}"
    )
    ...
    print(
        f"standard deviation of weekly returns: {acc_curve_group['assettrend8'].gross.weekly.std()}"
    )
    print(
        f"annualised std. deviation of daily (net) returns: {acc_curve_group['carry30'].daily.ann_std()}"
    )
    print(
        f"median of annual costs: {acc_curve_group['mrinasset160'].costs.annual.median()}"
    )
