...
def adjust_weights_with_factor(
    ...
    rounded_adjusted_trades_as_weights = (
        calculate_adjusting_trades_rounding_in_min_bet_space(
...
def calculate_adjusting_trades_rounding_in_min_bet_space(
    ...
    rounded_adjusted_trades = [
        round(round(x / min_bets_as_np[idx]) * min_bets_as_np[idx], 2)
        for idx, x in enumerate(adjusted_trades)
    ]

    rounded_as_weights = rounded_adjusted_trades * (
        per_contract_value_as_np / min_bets_as_np
    )

    return rounded_as_weights
