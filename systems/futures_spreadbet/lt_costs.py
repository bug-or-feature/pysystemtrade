...
def get_spreadbet_costs():
    ...
    system = System([Account(), Rules(), FuturesSpreadbetRawData()], sim, config)
    ...
    for instr in instr_list:
        ...
        # not used in LT?
        daily_percentage_volatility = system.rawdata.get_daily_percentage_volatility(
            instr
        )
        ...
        trading_capital = CAPITAL_PER_INSTR + get_current_pandl(
            instr, positions, ig_prices
        )
        ideal_exposure = (
            (rescaledForecast / 10) * INSTR_TARGET_RISK * trading_capital
        ) / annual_vol
        ...
        adjustment_required = (
            adjustment_calc(sb_price, current_exposure, ideal_exposure)
