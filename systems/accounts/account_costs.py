...
class accountCosts(accountInputs):
    ...
    def get_SR_cost_for_instrument_forecast(
        ...
        use_pooled_costs = str2Bool(self.config.forecast_cost_estimates[
                                        "use_pooled_costs"
                                    ])

        if use_pooled_costs:
            SR_cost = self._get_SR_cost_for_rule_with_pooled_costs(instrument_code,
                                                                   rule_variation_name)

        else:
            SR_cost = self._get_SR_cost_of_rule_for_individual_instrument(instrument_code,
                                                                          rule_variation_name)
        return SR_cost

    @input
    def _get_SR_cost_for_rule_with_pooled_costs(self, instrument_code: str,
                                                rule_variation_name: str) -> float:
    ...
    def _get_SR_cost_instr_forecast_for_list(
        ...
        list_of_SR_cost = [self._get_SR_cost_of_rule_for_individual_instrument(instrument_code,
                                                                               rule_variation_name)
                           for instrument_code in instrument_code_list]

        # weight by length
        cost_weightings = self._get_forecast_length_weighting_for_list_of_instruments(instrument_code_list,
                                                                                     rule_variation_name)
    ...
    @diagnostic()
    def _get_forecast_length_weighting_for_list_of_instruments(self, instrument_code_list: list,
                                                              rule_variation_name: str) -> list:

        forecast_lengths = [self._get_forecast_length_for_instrument_rule(instrument_code,
                                                                         rule_variation_name)
                            for instrument_code in instrument_code_list]
    ...
    @diagnostic()
    def _get_forecast_length_for_instrument_rule(self, instrument_code: str,
    ...
    def forecast_turnover(self, instrument_code: str,
        ...
        if use_pooled_turnover:
            ...
            turnover = self._forecast_turnover_for_individual_instrument(instrument_code,
                                                                         rule_variation_name)

        return turnover

    @diagnostic()
    def _forecast_turnover_pooled(self, instrument_code: str,
                                 rule_variation_name: str) -> float:
    ...
    def _get_SR_cost_per_trade_for_instrument_percentage(self, instrument_code: str) -> float:
        ...
        cost_in_currency_terms = \
            raw_costs.calculate_cost_instrument_currency(notional_blocks_traded,
