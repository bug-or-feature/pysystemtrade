...
def plot_performance(log, system, optimised=False):
    if optimised:
        portfolio = system.accounts.optimised_portfolio().percent
    else:
        portfolio = system.accounts.portfolio().percent
    log.info(f"Stats as %: {portfolio.stats()}\n")
    portfolio.curve().plot(legend=True)
    show()
